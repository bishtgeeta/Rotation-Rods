...
#######################################################################
inputFile = ''
outputFile = r'Z:\Geeta-Share\Rotation\20170619102035\20170619102035.h5'
inputDir = r'Y:\seewee\Shufen\16-32-45.800_Export\20170619102035'
outputDir = r'Z:\Geeta-Share\Rotation\20170619102035\output'
...
#########
    ...
    #frameList = range(0,50)
    #numFrames = len(frameList)#frameList[-1]
    #for frame in tqdm(frameList):
        #gImg = cv2.imread(inputDir+'/16-32-45.800_'+str(frame).zfill(0)+'.tif',0)
...
for frame in tqdm(procFrameList[rank]):
    ...
    blobs_log = blob_log(gImgProc, min_sigma=8, max_sigma=10, num_sigma=5, threshold=0.14)
    #bImg =  gImgRaw.copy(); bImg[:]=0
    ...
    if (blobs_log.size > 0):
        ...
        for r,c,rad in blobs_log:
            ...
            if ((rr<0).any()==True or (cc<0).any()==True):
                ...
                #bImg[rr,cc] = 255
    #plt.figure(), plt.imshow(numpy.maximum(bImg,gImgRaw)), plt.show()
